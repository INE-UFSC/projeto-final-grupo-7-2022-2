...
class BarraDeslocamento:
    def __init__(self, altura):
        ...
        self.__rect_barra = pg.Rect(self.__configuracoes.largura_tela - 20, 20, 20, altura_barra)
    ...
    def atualizar(self, eventos):
        for evento in eventos:
            ...
            if evento.type == pg.MOUSEBUTTONDOWN:
                ...
                if self.__rect_barra.collidepoint(pos):
                    self.__diferenca_mouse = pos[1] - self.__rect_barra.y
        ...
        scroll_length = self.__configuracoes.altura_tela - self.__rect_barra.height - 40
        bar_half_lenght = self.__rect_barra.height / 2 + 20

        if self.__na_barra:
            pos = pg.mouse.get_pos()
            self.__rect_barra.y = pos[1] - self.__diferenca_mouse
            if self.__rect_barra.top < 20:
                self.__rect_barra.top = 20
            elif self.__rect_barra.bottom > (self.__configuracoes.altura_tela - 20):
                self.__rect_barra.bottom = self.__configuracoes.altura_tela - 20

            self.__eixo_y = int(diferenca_altura / (scroll_length * 1.0) * (self.__rect_barra.centery - bar_half_lenght) * -1)
        else:
            self.__rect_barra.centery = scroll_length / (diferenca_altura * 1.0) * (self.__eixo_y * -1) + bar_half_lenght

    def desenhar(self, superficie):
        pg.draw.rect(superficie, (197, 194, 197), self.__rect_barra)
